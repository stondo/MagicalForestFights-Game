...
TEST_F(MagicalCreatureTest, MagicalCreaturesStatsAreInExpectedRanges) {
    ...
    EXPECT_GE(natelus.GetLuck(), get<0>(hero_luck_tpl) / 100.0);
    EXPECT_LE(natelus.GetLuck(), get<1>(hero_luck_tpl) / 100.0);
    ...
    EXPECT_GE(wild_beast.GetLuck(), get<0>(wildbeast_luck_tpl) / 100.0);
    EXPECT_LE(wild_beast.GetLuck(), get<1>(wildbeast_luck_tpl) / 100.0);
}
